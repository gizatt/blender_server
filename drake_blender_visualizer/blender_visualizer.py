...
from pydrake.geometry import Role, SceneGraph, QueryObject
...
class BlenderColorCamera(LeafSystem):
    ...
    def __init__(self,
        ...
        self._bbox_bundle_input_port = self.DeclareAbstractInputPort(
            "bbox_bundle", AbstractValue.Make(BoundingBoxBundle(0)))
    ...
    def load(self):
        ...
        for frame_id in inspector.GetAllFrameIds():
            ...
            for j, g_id in enumerate(inspector.GetGeometries(frame_id, self._role)):
                ...
                geom_name = name + "::" + "%d" % j
                X_FG = inspector.GetPoseInFrame(g_id)
                tfs.append(X_FG)

                geom_position = X_FG.translation().tolist()
    ...
    def DoPublish(self, context, event):
        ...
        if self._bbox_bundle_input_port.HasValue(context):
            bbox_bundle = self._bbox_bundle_input_port.Eval(context)
        else:
            bbox_bundle = None
        ...
        for frame in self._dynamic_frames:
            ...
            for j, tf in enumerate(frame["local_tfs"]):
                ...
                location = geom_WF.translation()
                quaternion = geom_WF.rotation().ToQuaternion().wxyz()
                geom_name = name + "::" + "%d" % j
                self.bsi.send_remote_call(
                    "update_object_parameters",
                    name="obj_" + geom_name,
